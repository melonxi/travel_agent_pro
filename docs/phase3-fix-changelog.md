...
Phase 3（逐日行程落地）在把 Phase 2 选定的骨架方案展开为逐天行程时，表现极不稳定：
...
### 1.2 根因：Phase 3 的 LLM 是「瞎子」

深入代码后发现，根本原因是 **Phase 3 的 LLM 看不到它需要展开的骨架内容**。

Phase 2 → Phase 3 切换时，系统做了两件事：
...
所以 Phase 3 LLM 的处境是：**知道选了 plan_A，但不知道 plan_A 里写了什么**。它只能靠猜。

具体信息丢失清单：

| 信息 | Phase 2 能看到 | Phase 3 能看到 | 影响 |
|------|--------------|--------------|------|
...
Phase 2 已有 `_build_phase2_state_repair_message()`——当 LLM 只输出文本不写状态时自动提醒。但 Phase 3 **完全没有类似机制**，导致 LLM 口头描述完行程就「以为完成了」。
...
### 修复 2：Phase 3 State Repair（agent/loop.py）
...
- 与 Phase 2 repair 共享 `repair_hints_used` 防重入机制

### 修复 3：Phase 3 Prompt 增强（phase/prompts.py）

在 Phase 3 系统提示词中新增三段关键指引：
...
```
Phase 2 repair 检查的键：step（如 "brief"）
Phase 3 repair 检查的键："p3_daily"
主循环实际添加的键：f"p{current_phase}_{phase2_step}"
                      → 对 Phase 2 是 "p2_brief"
                      → 对 Phase 3 是 "p3_daily"
```

**后果**：键永远匹配不上，repair hint 会**无限重复注入**，直到 `max_retries` 耗尽。Phase 2 和 Phase 3 的 repair 去重都是坏的。
...
当存在 `plan_A` 和 `plan_A_plus` 两个骨架时，选中 `plan_A` 可能错误匹配到 `plan_A_plus`。注入了**错误的骨架**意味着 Phase 3 LLM 会按错误的方案排行程，比不注入更危险。
...
```python
# Phase 2 repair - 检查和添加使用相同格式
repair_key = f"p2_{step}"  # 如 "p2_brief"
...
if current_phase == 3:
    repair_hints_used.add("p3_daily")
else:
...
| 文件 | 变更 | 说明 |
|------|------|------|
| `backend/context/manager.py` | 改进 | Phase 3+ 骨架/画像/偏好/约束/进度注入；`_find_selected_skeleton` 安全匹配 |
| `backend/agent/loop.py` | 新增+修正 | Phase 3 repair 机制；去重键修正；JSON/日期检测；唯一天数 |
| `backend/phase/prompts.py` | 增强 | Phase 3 prompt 增加输入来源/增量指引/最重要提醒 |
...
| `docs/phase3-skeleton-injection-fix.md` | 新增 | 详细根因分析文档 |
| `docs/phase3-fix-changelog.md` | 新增 | 本文档 |

---

## 六、关键设计决策

1. **Phase 条件注入**：仅 Phase 3+ 注入完整内容。Phase 2 LLM 自己生成骨架，不需要重注入。
...
```
1a3b50c 🐛 fix: address critique findings for Phase 3 repair and skeleton matching
37f1600 🐛 fix: inject skeleton content into Phase 3 context for stable daily plan generation
...
| 骨架 ID 归一化 | 低 | 在 Phase 2 写入骨架时即标准化 ID 格式，减少 Phase 3 匹配复杂度 |
| 更多 repair 格式支持 | 低 | 支持 Markdown 表格格式的行程输出检测 |
| E2E 全链路测试 | 中 | 当前 Playwright 测试只覆盖 Phase 1 流程，应扩展到 Phase 2→3 |
