...
> 状态：`Stage 0`、`Stage 1`、`Stage 2`、`Stage 3` 已落地；系统当前已统一 `RecallCandidate` contract，但 `EpisodeSlice` 仍走 legacy query 判定，`Stage 4/5` 的 reranker-select 流水线仍未落地
...
- 当前已实现范围不包含 reranker
- 当前 recall gate 不升级为“完整 phase / plan / 全量历史消息”上下文推理器，仍保持轻上下文输入
...
- `Stage 2 -> Stage 3` 之间的 adapter 兼容层已落地
- `Stage 3` 已统一收敛到 `RecallCandidate` 输出 contract，但内部仍沿用现有 `rank_profile_items()` / `rank_episode_slices()` 规则逻辑
- `EpisodeSlice` 仍未纳入统一 retrieval plan，而是保留 legacy query 判定能力
- 当前 formatter 已能消费统一 candidate，但仍是“Stage 3 候选直接注入 prompt”，`Stage 4 Reranker` 与 reranker-aware `Stage 5` 仍未落地

当前代码落点可概括为：

- `backend/main.py`
  - 负责 `Stage 0` 短路、`Stage 1` gate、`Stage 2` query tool 的 LLM 编排
  - 负责把 `memory_recall` internal task / SSE telemetry 写回 chat 主链路
- `backend/memory/manager.py`
  - 负责 fixed profile + working memory 注入
  - 负责把 `retrieval_plan` 接到 `rank_profile_items()`，并把 legacy slice query 接到 `rank_episode_slices()`
  - 当前在拿到统一 `recall_candidates` 后直接交给 formatter
- `backend/memory/retrieval_candidates.py`
  - 负责把 profile / slice 规则召回结果正规化为统一 `RecallCandidate`
- `backend/memory/formatter.py`
  - 负责最终 markdown context 与 `MemoryRecallTelemetry`
- `backend/api/trace.py` + `backend/telemetry/stats.py`
  - 当前只序列化 recall telemetry 的摘要字段，尚未承载 reranker 结果
...
| Stage 1 Recall Gate | 要不要 recall | 最近用户窗口 + 轻量 trip facts + 记忆概览 | `needs_recall / intent_type / reason / confidence` | 不产出 retrieval plan |
...
当前 gate 实际输入不是“只看单条 user message”，而是一个轻量上下文包：

- 最近用户消息窗口（不是完整对话）
- 当前旅行事实摘要（从 `TravelPlanState` 抽取）
- 现有记忆概览（profile / working memory / slice 的极简 summary）
...
- 比“只看单条 message”更能区分“当前行程事实”与“调用旧记忆”
- 仍保持输入足够轻，避免 gate 变成第二个大问答模型
- 与当前 `main.py::_decide_memory_recall()` 的真实实现一致
...
- 因此 Milestone D 不能假设 query tool 已经为 `episode_slice` 提供了统一 plan；reranker 必须消费 manager 侧已经合并好的 candidate 列表
...
- 为了兼容 `Stage 2`，当前通过 `allowed_buckets` / `strictness` 接入 retrieval plan
- `Stage 3` 已改造成文档 §6.3 中的统一 `RecallCandidate` 输出
- `manager` / `formatter` / trace/stats 已消费统一 candidate，而不再区分 profile tuple / slice tuple 双轨 contract

以下 `10.2 ~ 10.4` 描述的是 **Stage 3 当前保留的规则检索与评分约束**。Milestone D 不应重写这些逻辑，而应把 reranker 插在 candidate 产出之后。
...
### 10.5 对 Milestone D 的设计影响

- `RecallCandidate` 已经是当前稳定输入 contract，Milestone D 不需要再承担 Stage 3 收敛工作
- `candidate.score` 当前是**当前候选列表内部的 ordinal normalized rank**，只保证相对顺序，不保证跨 query / 跨 source 的绝对可比性；reranker 只能把它当排序提示，不能把它当语义分数直接阈值裁剪
- `manager.generate_context()` 当前是在合并出 `recall_candidates` 后直接交给 formatter 生成 `## 本轮请求命中的历史记忆`；Milestone D 的最小插入点应该在 `recall_candidates -> selected_candidates -> format_v3_memory_context(...)` 之间
- 由于 `EpisodeSlice` 仍走 legacy slice query，Milestone D 的 reranker 输入必须以“统一 candidate 列表”为边界，不能反向要求 query tool 先统一支持 `episode_slice`
...
- `user_message`
- `retrieval_plan`
- 统一 `RecallCandidate[]`（来自 `rank_profile_items()` / `rank_episode_slices()` 合并后的 top K 候选）
- 最小当前行程摘要（从 `TravelPlanState` 提取 destination / dates / skeleton / active constraints 的轻量视图）

说明：

- reranker 的直接输入边界应以 `MemoryManager.generate_context()` 现有可用数据为准，避免为 Milestone D 引入新的全局依赖
- 当前 `RecallCandidate.content_summary`、`matched_reason`、`domains`、`applicability` 已足够支撑第一版 reranker prompt，不需要回读完整 profile item / episode slice 原文
...
- 第一版只允许从输入 candidate 中做子集选择，不允许新增候选或改写 `item_id`
- 第一版保持 source-agnostic：`profile` / `episode_slice` 共用同一套 reranker 输出 contract
- 如果候选总数 `<= 3`，允许直接短路跳过 reranker，沿用原顺序进入 Stage 5，以控制额外延迟
...
- `candidate_count = 0`
  - 不调用 reranker，直接结束 recall select
- `candidate_count <= 3`
  - 允许直接沿用原排序结果，记录 `skipped_small_candidate_set`

这样做的原因：

- 小候选集上的 reranker 信息增益通常有限
- 当前 recall 已经额外有 gate + query 两次 LLM 调用，Milestone D 应优先控制尾延迟
...
- formatter 已能消费 `RecallCandidate[]` 并渲染统一 recall block
- Milestone D 不需要重写 formatter 的渲染骨架，只需要把输入从“全部 `recall_candidates`”切到“`selected_candidates` 或 fallback top N”
- 因此前端/trace 的主要新增工作不在 markdown 结构，而在 telemetry 字段补全与结果可视化
- 具体插入点应是：

```text
recall_candidates
  ↓
select_recall_candidates (new)
  ↓
selected_candidates
  ↓
...
- 但三条观测链路的字段完备度并不相同：
  - chat SSE `memory_recall` 事件当前拿到的是 `MemoryRecallTelemetry.to_dict()` 全量字段
  - `SessionStats.recall_telemetry` 当前只保留 `stage0/gate/final/fallback` 摘要
  - `trace.py` 也只序列化这部分摘要，因此 Trace API 还看不到 `query_plan`、候选数或未来的 reranker 细节
- 前端当前状态也分裂：
  - `ChatPanel` 已消费 `memory_recall` SSE，并基于 id/source 生成 inline 提示
  - `TraceViewer` 的类型与 UI 目前还没有正式展示 `memory_recall` 细节
...
- 现有 SSE / stats / trace / ChatPanel 只消费 `sources`、命中 id 列表与 gate 决策，尚未暴露 candidate_count / reranker 结果
- 因此 Milestone D 不能只改后端 recall 逻辑；若要保持可观测性闭环，必须同步扩展 `MemoryRecallTelemetry`、`memory_recall` SSE payload、trace API 与前端类型
...
### Milestone D：Stage 4（下一步）

- 引入 LLM reranker
- 从统一 `RecallCandidate[]` 中选出最终注入项
- 将 Stage 5 输入改为 `selected_candidates` / fallback top N，而非全部候选

目标：

- 降低 prompt 污染，提高本轮相关性
- 保持对 `profile` / `episode_slice` 的 source-agnostic 最终裁决
- 在 SSE / trace / stats 中补齐 reranker 过程可观测性

本阶段非目标：

- 不重写 `rank_profile_items()` / `rank_episode_slices()` 的内部规则模型
- 不把 `episode_slice` 提前并入统一 query tool
- 不改变 fixed profile / working memory 的注入策略
- 不为 reranker 引入“跨 source 绝对分数”体系

建议文件边界：

- 新增 `backend/memory/recall_reranker.py`
  - reranker tool schema
  - payload 解析
  - fallback 选择逻辑
- 修改 `backend/memory/manager.py`
  - 在 `recall_candidates` 与 formatter 之间插入 reranker
  - 产出 `selected_candidates` 与 reranker telemetry
- 修改 `backend/memory/formatter.py`
  - 保持渲染骨架，消费 reranker 选中的 candidate
- 修改 `backend/main.py`
  - 扩展 `memory_recall` internal_task / SSE payload / stats 映射
- 修改 `frontend/src/types/trace.ts`
  - 补齐 reranker 相关字段
- 修改 `frontend/src/components/ChatPanel.tsx`
  - 在记忆召回系统任务卡中展示 reranker 摘要
- 修改 `frontend/src/components/TraceViewer.tsx`
  - 展示 candidate_count / selected ids / fallback 信息

建议新增 manager-side 选择函数边界：
...
- 该函数只做“从已有 candidates 里挑子集”
- 不回写 store，不改 candidate 原始排序
- 可以在内部做 `skip / llm / fallback` 三路选择，但对外统一返回 `selected_candidates + telemetry`
...
3. `Stage 4` 是否需要把“候选数很少”的 case 短路跳过 reranker，以换取更低延迟
4. Gate 是否长期保持“最近用户窗口 + 轻量 trip facts + 记忆概览”的轻上下文输入，还是后续升级为更完整的 phase / conversation state
...
- `Stage 0 + Stage 1 + Stage 2 + Stage 3` 已落地
- `Stage 2` 通过 adapter 接入当前规则召回器
- 系统当前处于“新 query tool + 已统一 candidate 的 Stage 3 + 旧 EpisodeSlice query”过渡态

因此后续工作的重点已经从“是否要做 gate / query tool”转为：

- 在统一 candidate contract 之后插入 Stage 4 reranker
