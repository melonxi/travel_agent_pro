...
我会优先展示一条从 Phase 3 skeleton 到 Phase 5 并行 handoff 的 trace，再配合 `backend/agent/loop.py` 和 `backend/agent/phase5/parallel.py`。原因是它同时体现 Agent 工程的几个关键能力：阶段推进、工具写状态、并行 workers、全局验证、内部 `replace_all_day_plans` handoff、SSE 进度和 trace 归因。单看最终答案不如看这条 trajectory 有说服力。
...
本次 `pytest --collect-only -q` 可收集到 `1692 tests`，但没有跑全量测试，所以面试时应说“当前可收集测试数量”，不要说“全部通过”。
