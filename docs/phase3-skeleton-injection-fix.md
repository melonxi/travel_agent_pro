# Phase 3 骨架注入修复：逐日细化不稳定问题根因分析与解决
...
Phase 3（逐日行程落地与验证）在对 Phase 2 构建的旅行骨架进行逐天细化时，表现出严重的不稳定性。具体现象包括：

- LLM 生成的逐日行程与用户在 Phase 2 选定的骨架方案不一致
...
Phase 3 是「最后一公里规划器」，核心职责是将 Phase 2 产出的已选骨架方案（skeleton）展开为覆盖全部日期的 `daily_plans`。工作流程为四步：
...
阶段判定条件：`len(plan.daily_plans) < plan.dates.total_days` 则停留在 Phase 3。

## 根因分析

### 核心问题：Phase 3 的 LLM 看不到它需要展开的骨架内容

当 Phase 2 → Phase 3 切换发生时，LLM 的上下文中存在严重的信息缺失：

| 信息 | 切换前（Phase 2 LLM 能看到） | 切换后（Phase 3 LLM 能看到） | 缺失影响 |
|------|------|------|------|
| 已选骨架内容 | 完整的骨架方案数据结构 | 仅 `已选骨架：plan_A`（ID） | **致命**：Phase 3 无法知道骨架安排了什么 |
...
Phase 3 的 LLM 只能看到「有 3 套骨架方案，选了 plan_A」，完全不知道 plan_A 里安排了什么区域、什么景点、什么主题。

#### 2. `compress_for_transition()` 截断关键数据

Phase 2 → 5 的转场摘要使用 `_short_repr()` 将工具调用数据截断到 160 字符。skeleton_plans 这种大型结构化数据被截断后不可用。

#### 3. 缺乏 Phase 3 专用的 state repair 机制

Phase 2 有 `_build_phase2_state_repair_message`，当 LLM 只输出文本不写状态时自动提醒。Phase 3 完全没有这个机制，导致 LLM 在自然语言中描述了每天的行程后就「以为完成了」，不再调用 `update_plan_state` 写入结构化数据。
...
```python
# 修复后：Phase 3+ 输出具体字段，Phase 2 保持计数
if plan.trip_brief:
    if plan.phase >= 5:
        parts.append("- 旅行画像：")
...
### 4. Phase 3 State Repair（`backend/agent/loop.py`）

新增 `_build_phase3_state_repair_message()` 方法：

- 检测 Phase 3 LLM 输出中是否包含逐日行程文本（通过正则匹配「第N天」「Day N」「HH:MM」等模式）
- 如果检测到行程文本但 `daily_plans` 仍不完整，注入提醒消息
- 提醒 LLM 必须调用 `update_plan_state(field="daily_plans", value=...)` 写入结构化数据
- 与 Phase 2 repair 共享同一防重入机制（`repair_hints_used`）

### 5. Phase 3 Prompt 增强（`backend/phase/prompts.py`）
...
| 测试 | 覆盖点 |
|------|--------|
| `test_phase3_runtime_context_injects_selected_skeleton` | 验证选中骨架完整内容注入 |
| `test_phase3_runtime_context_injects_trip_brief_content` | 验证旅行画像字段级注入 |
| `test_phase3_runtime_context_injects_preferences_and_constraints` | 验证偏好/约束注入 |
| `test_phase3_runtime_context_shows_daily_plans_progress` | 验证已完成天数摘要和待规划天数 |
| `test_phase2_runtime_context_shows_count_only` | 回归：Phase 2 不受影响 |
...
| 测试 | 覆盖点 |
|------|--------|
| `test_phase3_text_only_daily_plan_triggers_state_repair` | 验证 Phase 3 repair 机制：LLM 只输出文本 → 注入提醒 → LLM 写入 daily_plans |
...
1. **为什么只在 Phase 3+ 注入详细内容？** Phase 2 对话中 LLM 自己生成了骨架，不需要重新注入。Phase 3 是全新的 LLM 上下文（经过转场压缩），需要看到前一阶段的完整产出。
...
4. **为什么 Phase 2 保持计数格式？** Phase 2 中骨架正在被 LLM 自己构建，注入完整内容反而会让上下文膨胀且无额外价值。

## 文件变更清单

| 文件 | 变更类型 | 说明 |
|------|---------|------|
| `backend/context/manager.py` | 改进 | `build_runtime_context` 增加 Phase 3 骨架/画像/偏好/约束/进度注入；新增 `_find_selected_skeleton` |
| `backend/agent/loop.py` | 新增 | `_build_phase3_state_repair_message` 方法；repair 链路整合 |
| `backend/phase/prompts.py` | 增强 | Phase 3 prompt 增加输入来源说明、增量生成指引、最重要提醒 |
| `backend/tests/test_context_manager.py` | 新增 | 9 个 Phase 3 注入相关测试（含歧义骨架 ID 安全测试） |
| `backend/tests/test_agent_loop.py` | 新增 | 3 个 Phase 3 repair 测试（含去重、JSON 格式检测） |
| `docs/phase3-skeleton-injection-fix.md` | 新增 | 本文档 |
...
- **问题**：Phase 2 repair 检查 `step`（如 `"brief"`），Phase 3 检查 `"p3_daily"`，但主循环统一添加 `f"p{phase}_{phase2_step}"`。键永远匹配不上，导致 repair hint 无限重复注入。
- **修复**：Phase 2 使用 `f"p2_{step}"`；Phase 3 使用 `"p3_daily"`；主循环根据 `current_phase` 添加对应键。
