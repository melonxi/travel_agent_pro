...
| 多 Agent / RAG | 中 | 缺失 | **40%** | 面试 Q10/Q12 会暴露此缺口，只能概念性回答 |
...
**第三，无多 Agent / RAG 经验（+7% 失败概率）**

面试系统设计环节若问"怎么设计多 Agent 协作"或"怎么集成领域知识库"，目前只能概念性回答，无代码支撑。这是 TAP 作为单 Agent 系统的结构性缺口。

#### 提升概率的最优路径

完成以下两项，征服概率从 **80% → 87-90%**：

1. **运行一次完整 eval + 生成报告**（1-2 天）——消除"有枪没子弹"的最大质疑，无需写新代码
2. **修复 3 个失败场景并记录前后对比**（2-3 天）——完成"发现→分析→修复→验证"完整闭环

这两件事无需新建任何能力，只需用好已有工具产出数据。
