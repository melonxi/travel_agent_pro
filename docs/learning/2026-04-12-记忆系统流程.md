...
本文描述当前 Travel Agent Pro 的 v3 记忆系统：system prompt 只注入分层后的长期画像、会话工作记忆，以及按查询触发的历史符号召回；当前旅行事实仍由 `TravelPlanState` 权威提供。

## 总览

当前记忆系统是一个异步、分层、带审核策略的记忆层：

- 每轮对话开始前：`MemoryManager.generate_context()` 按 v3 架构组装 prompt 记忆区块。
...
flowchart TD
    ...
    E --> F1[load_profile -> 固定长期画像召回]
    E --> F2[load_working_memory -> 当前会话工作记忆]
    ...
    R -- working_memory --> T[写入 sessions/<session_id>/working_memory.json]
    R -- drop --> U[丢弃]
...
|------|------|------------------|------|
| profile fixed recall | `profile.json` 中的 `constraints` / `rejections` / `stable_preferences` | 每轮固定召回 | 稳定长期偏好、约束、长期排除项 |
| working memory recall | 当前 `session_id + trip_id` 的 working memory | 每轮固定召回 | 当前会话里的临时拒绝、待避免方向、阶段性提醒 |
| query-aware symbolic recall | `rank_profile_items()` + `rank_episode_slices()` | 只在用户显式询问历史经验、以往选择、长期偏好时触发 | 把与当前问题直接相关的历史偏好和 episode slice 补进上下文 |
...
#### 1. Profile fixed recall

`MemoryManager._fixed_profile_items()` 依次读取：

1. `constraints`
2. `rejections`
3. `stable_preferences`

只取 `status == active` 的条目，并按固定上限注入 prompt。
...
当前用户记忆目录同时保留 v3 主存储与少量 v2 兼容文件：

```mermaid
flowchart TD
    A[data/users/{user_id}/] --> B[memory/]
    A --> C[memory.json]
    A --> D[memory_events.jsonl]
    A --> E[trip_episodes.jsonl]

    B --> B1[profile.json]
    B --> B2[sessions/{session_id}/working_memory.json]
...
- `memory/sessions/<session_id>/working_memory.json` 是 v3 会话工作记忆主存储。
- `memory/episode_slices.jsonl` 是 v3 历史切片主存储。
- `memory.json` 和 `trip_episodes.jsonl` 目前仍主要用于兼容旧 API、迁移或归档读取，不再是生产 prompt assembly 的主依赖。
...
- 旧数据读取与 pending 流程
- 前端尚未切到独立 v3 写接口前，对 v3 profile item 的确认/拒绝/删除代理写入

这不代表 prompt assembly 仍依赖 v2 runtime；它只说明写入侧还保留一层过渡兼容壳。
...
当前记忆系统的核心模式是：每轮前固定召回 profile 与 working memory，并只在显式历史/偏好问题上触发 query-aware symbolic recall；每轮后异步抽取并写入 v3 分层存储；当前旅行事实始终由 `TravelPlanState` 权威提供，不再依赖 v2 的广义 trip memory prompt 注入。
