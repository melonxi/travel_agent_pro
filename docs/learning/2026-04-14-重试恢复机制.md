...
### 8.0 `/chat`、`/cancel`、`/continue` 三接口关系图

```mermaid
flowchart TD
    A[用户输入新消息] --> B[POST /api/chat/{session_id}]
    B --> C[开始一轮新的 Agent 运行]
    C --> D{运行结果}

    D -->|正常完成| E[SSE done]
    D -->|失败但可继续| F[SSE error\ncan_continue=true]
    D -->|失败可重试但不可继续| G[SSE error\nretryable=true, can_continue=false]
    D -->|不可恢复失败| H[SSE error\nretryable=false, can_continue=false]
    D -->|用户主动取消| I[SSE done\nrun_status=cancelled]

    F --> J[POST /api/chat/{session_id}/continue]
    J --> K[读取 RunRecord.continuation_context]
    K --> L[恢复中断的这一轮]

    C --> M[POST /api/chat/{session_id}/cancel]
    M --> N[设置 session._cancel_event]
    N --> I
