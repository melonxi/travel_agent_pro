...
在通过 Playwright MCP 做 Phase 1 → Phase 4 全流程回归时，发现 Phase 3 的"逐日行程提交"环节存在若干串联性 bug：LLM 在产出 `daily_plans` 时字段形状有一定随机性，而后端 dataclass 的 `from_dict` 只处理"理想输入"，导致一遇到变体就抛错，进而让 Phase 3 无法进入 Phase 4；随后 Phase 4 的 `generate_summary` 又在另一套宽松输入上继续抛错。本文档汇总这批问题的根因、修复方式以及与之配套的测试。

## 观察到的现象

- Phase 3 中 `update_plan_state(field="daily_plans", value=...)` 频繁抛 `TypeError: string indices must be integers, not 'str'`，或 `KeyError: 'category'`。
- Phase 3 即使前端看起来"写入完成"，Plan 的 `daily_plans` 常常是空的，导致 Phase 2 → 4 自动转阶段迟迟不触发。
- Phase 4 `generate_summary` 间歇性抛 `TypeError: object of type 'int' has no len()`，或在 `budget` 不是 dict 时崩溃。
...
5. Phase 3 prompt 里对 `daily_plans` 的字段形状只描述了"每天应该包含哪些字段"，没有写清"`location` 必须是 dict"、"`category` 必须提供"这类硬约束；`update_plan_state` 的参数描述也只字未提 `daily_plans` 的嵌套结构，导致 LLM 没有学习信号纠正自己。
...
### 3. Phase 3 Prompt 硬约束（`backend/phase/prompts.py`）

在 Phase 3 prompt 的 `assemble` 小节追加了 `DayPlan` 的**严格 JSON 结构示例**和一份硬约束清单：
...
这组约束给 LLM 明确的"什么是对的""什么会被丢弃"的信号。配合宽容化的 dataclass，即使个别字段仍然漂，Phase 3 也不会被单条异常打断。
...
### 已有的 Phase 2 → 4 集成 / E2E 回归

- `tests/test_appendix_issues.py::TestA2DailyPlansBlocked::test_phase3_to_4_transition_after_daily_plans`：覆盖 `update_plan_state → DayPlan.from_dict → phase_router` 的 transition 链路。
- `tests/test_e2e_golden_path.py`：模拟 LLM stub 驱动 Phase 1→2→3→4 全流程，包含 `daily_plans` 5 天的整体 list 提交。
...
tests/test_phase24_merge.py         22 passed
...
Phase 1 → 3 的工具调用链路与前端渲染均正常，配合前面的 golden path E2E 覆盖 Phase 2→4 transition，本次修复的完整链路回归闭环。
...
- **为什么 Phase 3 prompt 改得比较"硬"？** Phase 3 的产物要直接落到结构化 DAG 里，没有模糊空间；而 Phase 1/3 的产物本身就更偏对话，不需要 schema 级硬约束。两边的宽严梯度是刻意的。
...
- `backend/phase/prompts.py`：Phase 3 prompt 追加严格 JSON 结构示例与硬约束。
