...
async def test_memory_extraction_uses_forced_tool_call(app):
    ...
    assert observed["calls"][0]["tool_choice"] == {
        "type": "function",
        "function": {"name": "decide_memory_recall"},
    }
    ...
    assert observed["calls"][0]["tools"][0]["name"] == "decide_memory_recall"
    assert observed["calls"][1]["tools"][0]["name"] == "decide_memory_extraction"
    assert observed["calls"][2]["tools"][0]["name"] == "extract_memory_candidates"
