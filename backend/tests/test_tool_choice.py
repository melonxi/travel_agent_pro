...
def test_always_auto_after_migration(decider):
    ...
    plan = _make_plan(phase=2, phase2_step="brief")
    messages = [
    ...
    result = decider.decide(plan, messages, phase=2)
    assert result == "auto"


def test_phase3_itinerary_returns_auto(decider):
    """Phase 3 with itinerary text should return 'auto' after migration."""
...
def test_phase3_skeleton_returns_auto(decider):
    """Phase 2 skeleton step should return 'auto' after migration."""
    plan = _make_plan(phase=2, phase2_step="skeleton")
    messages = [
