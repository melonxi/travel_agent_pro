...
def test_phase3_brief_step():
    plan = TravelPlanState(session_id="s", phase=2, phase2_step="brief")
    assert "画像" in compute_narration(plan)


def test_phase3_candidate_step():
    plan = TravelPlanState(session_id="s", phase=2, phase2_step="candidate")
    assert "候选" in compute_narration(plan)


def test_phase3_skeleton_step():
    plan = TravelPlanState(session_id="s", phase=2, phase2_step="skeleton")
    assert "骨架" in compute_narration(plan)


def test_phase3_lock_step():
    plan = TravelPlanState(session_id="s", phase=2, phase2_step="lock")
    assert "锁定" in compute_narration(plan)


def test_phase3():
    plan = TravelPlanState(session_id="s", phase=3)
    assert "日程" in compute_narration(plan)


def test_phase4():
    plan = TravelPlanState(session_id="s", phase=4)
