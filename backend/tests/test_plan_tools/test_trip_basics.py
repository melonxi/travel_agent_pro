...
def test_trip_basics_tool_metadata(field_name, expected_any_of_types):
    ...
    assert {
        option["type"]
        for option in tool_fn.parameters["properties"][field_name]["anyOf"]
    } == expected_any_of_types
...
class TestUpdateTripBasics:
    ...
    # --- travelers edge cases: real LLM failure patterns ---

    @pytest.mark.asyncio
    async def test_accepts_travelers_pure_digit_string(self):
        """LLM passes travelers="2" (no unit suffix)."""
        plan = _make_plan()
        tool_fn = make_update_trip_basics_tool(plan)

        result = await tool_fn(travelers="2")

        assert result["updated_fields"] == ["travelers"]
        assert plan.travelers is not None
        assert plan.travelers.adults == 2
        assert plan.travelers.children == 0

    @pytest.mark.asyncio
    async def test_accepts_travelers_integer(self):
        """LLM passes travelers=2 (integer)."""
        plan = _make_plan()
        tool_fn = make_update_trip_basics_tool(plan)
    ...
    @pytest.mark.asyncio
    async def test_accepts_travelers_stringified_json(self):
        """LLM passes travelers='{"adults": 2}' (stringified JSON)."""
        plan = _make_plan()
        tool_fn = make_update_trip_basics_tool(plan)

        result = await tool_fn(travelers='{"adults": 2}')
    ...
    @pytest.mark.asyncio
    async def test_accepts_travelers_stringified_json_with_children(self):
        """LLM passes travelers='{"adults": 2, "children": 1}' (stringified JSON)."""
        plan = _make_plan()
        tool_fn = make_update_trip_basics_tool(plan)

        result = await tool_fn(travelers='{"adults": 2, "children": 1}')
    ...
    # --- dates edge cases: real LLM failure patterns ---

    @pytest.mark.asyncio
    async def test_accepts_dates_stringified_json(self):
        """LLM passes dates='{"start":"2026-05-01","end":"2026-05-05"}' (stringified JSON)."""
        plan = _make_plan()
        tool_fn = make_update_trip_basics_tool(plan)

        result = await tool_fn(dates='{"start": "2026-05-01", "end": "2026-05-05"}')

        assert result["updated_fields"] == ["dates"]
        assert plan.dates is not None
        assert plan.dates.start == "2026-05-01"
        assert plan.dates.end == "2026-05-05"
    ...
    @pytest.mark.asyncio
    async def test_accepts_budget_stringified_json(self):
        """LLM passes budget='{"total": 10000}' (stringified JSON)."""
        plan = _make_plan()
        tool_fn = make_update_trip_basics_tool(plan)

        result = await tool_fn(budget='{"total": 10000}')

        assert result["updated_fields"] == ["budget"]
        assert plan.budget is not None
        assert plan.budget.total == 10000.0

    @pytest.mark.asyncio
    async def test_accepts_budget_pure_digit_string(self):
        """LLM passes budget="10000" (pure digit string)."""
        plan = _make_plan()
        tool_fn = make_update_trip_basics_tool(plan)

        result = await tool_fn(budget="10000")
