...
PHASE3_BASE_RED_FLAGS: tuple[RedFlag, ...] = (
    RedFlag(
        id="P3-BASE-1",
    ...
    RedFlag(
        id="P3-BASE-4",
        trigger="你试图手动维护 phase3_step，而不是通过产物状态让系统自动推进。",
        repair="写入关键产物即可；不要尝试直接改子阶段。",
    ),
)


PHASE3_STEP_RED_FLAGS: dict[str, tuple[RedFlag, ...]] = {
    "brief": (
        RedFlag(
            id="P3-BRIEF-1",
    ...
    "skeleton": (
        ...
        RedFlag(
            id="P3-SKEL-3",
            trigger="骨架缺少后续 Phase 5 依赖的关键字段。",
...
PHASE5_WORKER_RED_FLAGS: tuple[RedFlag, ...] = (
    RedFlag(
        id="W5-1",
...
def build_active_red_flags(
    *,
    ...
    if worker:
        return PHASE5_WORKER_RED_FLAGS

    flags: list[RedFlag] = list(CORE_RED_FLAGS)
    if phase == 3:
        step = phase3_step or "brief"
        flags.extend(PHASE3_BASE_RED_FLAGS)
        flags.extend(PHASE3_STEP_RED_FLAGS.get(step, PHASE3_STEP_RED_FLAGS["brief"]))
    else:
