    ...
    if (event.from_phase !== event.to_phase) {
      ...
      ensureMemoryRecallTaskMessage({
        ...
        result: {
          ...
          stage0_decision: event.stage0_decision ?? '',
          stage0_matched_rule: event.stage0_matched_rule ?? '',
          query_plan_source: event.query_plan_source ?? '',
          query_plan_fallback: event.query_plan_fallback ?? 'none',
          recall_skip_source: event.recall_skip_source ?? '',
          candidate_count: candidateCount,
          recall_attempted_but_zero_hit: event.recall_attempted_but_zero_hit ?? false,
