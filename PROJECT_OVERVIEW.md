...
| Memory System | v3 profile / working memory / episode slice 分层记忆；当前旅行事实由 TravelPlanState 权威提供；query-aware symbolic recall 只在显式历史/偏好查询时触发；召回和后台提取以 `memory_recall` / `memory_extraction` 内部任务展示 | 每轮 chat 后后台提取；system prompt 构建前检索 |
