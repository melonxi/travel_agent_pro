...
| Memory System | 结构化长期/本次/episode 记忆，后台候选提取，policy 合并，按稳定 trip_id 隔离本次旅行记忆，受 `memory.enabled` 门控后阶段相关注入 | 每轮 chat 后后台提取；每次 system prompt 构建前检索 |
...
| Memory System | schema v2 结构化记忆；每轮 chat 后按 trigger 后台提取候选，policy 全字段 PII 阻断/脱敏/合并/确认；system prompt 前按 `memory.enabled` 和 trip_id 检索，Phase 7 幂等归档 episode |
