...
- **API 编排写后处理**：`backend/api/orchestration/agent/builder.py` 只装配 `AgentLoop`，`agent/tools.py` 负责工具注册，`agent/hooks.py` 注册实时校验、soft judge 和 quality gate hooks；`backend/api/orchestration/chat/stream.py` 负责 SSE 主循环，`chat/events.py` 负责事件序列化，`chat/finalization.py` 负责 run 收尾和保底持久化；`memory/orchestration.py` 组装记忆召回、提取、任务流和 episode 归档，`memory/contracts.py` 承载共享 dataclass 契约，`memory/recall_planning.py` 承载 recall gate fallback 与 retrieval-plan tool/prompt，`memory/extraction.py` / `memory/tasks.py` / `memory/episodes.py` 分别承载记忆提取、后台任务流和归档 episode 派生；`backend/api/routes/internal_task_routes.py` 承载 `/api/internal-tasks/...`，不再混入 `memory_routes.py`；`backend/api/orchestration/session/backtrack.py` 承载关键词回退与 reset trip 判定，`session/persistence.py` 承载消息/会话增量落盘与恢复（messages 表带 `phase`/`phase3_step` 列、`append_batch` 增量追加从不 DELETE 旧记录、`persisted_count` 跟踪已落盘条数；恢复路径区分 `history_view`（事实源全量）与 `runtime_view`（仅当前 phase/phase3_step 段）—— `runtime_view` 由纯函数 `session/runtime_view.derive_runtime_view` 从 history sidecar 标签派生，回退后旧 phase 内容绝不重放回 prompt，这是回退红线），`session/deliverables.py` 承载 Phase 7 交付物冻结，`common/telemetry_helpers.py` 与 `common/llm_errors.py` 承载 API 编排通用辅助逻辑。`AgentLoop` 在 phase / phase3_step 重建前会触发 `on_phase_rebuild` callback（实现位于 `agent/execution/phase_rebuild_callback.py`，从 loop.py 抽出以保持其在 size-guard 阈值之下），由 chat handler 用 rebuild 之前的 phase tag 落盘 tail 消息后再执行重建，避免新 prompt 与历史落盘标签错位。若工具结果显式返回 `previous_value` / `new_value`，`SessionStats.state_changes` 优先记录工具返回的规范化值；`request_backtrack` 只保留 rebuild/transition 语义，不伪造字段 diff
